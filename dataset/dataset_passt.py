...
#### This is a modified version of the AugmentMelSTFT class that supports mix-up training
class AugmentMelSTFT2(nn.Module):
    def __init__(self, n_mels=128, sr=441000, win_length=800, hopsize=320, n_fft=1024, freqm=48, timem=192,
                 htk=False, fmin=0.0, fmax=32000, norm=1, fmin_aug_range=1, fmax_aug_range=1000):
...
####################### Read the audio file and return the waveform #######################
def create_mix_up_dcase(data_path): 
    ...
    paired_devices = [] 
    for i in train_with_meta.groupby('file_number'):
        if len(i[1]) > 1 : 
            paired_devices.append(i[1])
    print("Paired devices found: ", len(paired_devices))
    return paired_devices , train_src_df, train_tgt_df, test_src_df, test_tgt_df

#create_mix_up_dcase('/home/abhishek/Code/M2D/Dcase-2020/TAU-urban-acoustic-scenes-2020-mobile-development/meta.csv')